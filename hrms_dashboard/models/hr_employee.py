...
class HrEmployee(models.Model):
    ...
    birthday = fields.Date(
        string="Date of Birth", groups="base.group_user", help="Birthday of employee"
    )

    def attendance_manual(self):
        """Create and update an attendance for the user employee and send Google Chat notification"""
        employee = (
            request.env["hr.employee"].sudo().browse(self.env.user.employee_id.id)
        )
        Attendance = request.env["hr.attendance"].sudo()

        # Get latest attendance before change
        last_attendance = Attendance.search(
            [("employee_id", "=", employee.id)], order="check_in desc", limit=1
        )
        was_checked_in = last_attendance and not last_attendance.check_out

        # Do the check-in/check-out operation
        ...
        # Decide what action was performed
        action = "Check Out" if was_checked_in else "Check In"
        ...
        if not webhook_url:
            _logger.warning("GOOGLE_CHAT_WEBHOOK_URL is not set.")
            return employee

        # Send to Google Chat
        try:
            message = {
                "text": f"👤 *{employee.name}* just performed *{action}* at `{timestamp}`."
            }
            requests.post(webhook_url, json=message, timeout=5)
        except Exception as e:
            _logger.warning("Failed to send Google Chat notification: %s", str(e))
    ...
    def get_user_employee_details(self):
        ...
        uid = request.session.uid
        employee = (
            self.env["hr.employee"].sudo().search_read([("user_id", "=", uid)], limit=1)
        )
        attendance = (
        ...
        timesheet_count = (
            ...
            .sudo()
            .search_count([("project_id", "!=", False), ("user_id", "=", uid)])
        )
        ...
        if employee:
            ...
            broad_factor = (
                result[0]["broad_factor"] if result[0]["broad_factor"] else False
            )
    ...
    def get_upcoming(self):
        ...
        for emp in birthday_employees:
            if (
                emp["birthday"].month == today.month
        ...
        cr.execute(
            """select e.id, e.name ->> e.lang as name, e.date_begin,
    ...
    def get_dept_employee(self):
        ...
        cr = self._cr
        cr.execute(
            """select department_id, hr_department.name,count(*)
        ...
        for i in range(0, len(dat)):
            data.append({"label": list(dat[i][1].values())[0], "value": dat[i][2]})
        return data
    ...
    def get_department_leave(self):
            ...
            for month in month_list:
                for line in result_lines:
                    if month.replace(" ", "") == line[0].replace(" ", ""):
                        match = list(
                            filter(lambda d: d["l_month"] in [month], graph_result)
                        )[0]["leave"]
        ...
        for result in graph_result:
            result["l_month"] = (
                result["l_month"].split(" ")[:1][0].strip()[:3]
                + " "
    ...
    def join_resign_trends(self):
        ...
        for i in range(11, -1, -1):
            ...
            month_list.append(text)
        for month in month_list:
            vals = {"l_month": month, "count": 0}
            join_trend.append(vals)
        for month in month_list:
            vals = {"l_month": month, "count": 0}
            resign_trend.append(vals)
        cr.execute(
            """select to_char(joining_date, 'Month YYYY') as l_month,
        ...
        cr.execute(
            """select to_char(resign_date, 'Month YYYY') as l_month,
         count(id) from hr_employee
    ...
    def get_employee_skill(self):
        ...
        employee = (
            ...
            .sudo()
            .search([("user_id", "=", request.session.uid)], limit=1)
        )
        skills = (
            self.env["hr.employee.skill"]
            .sudo()
            .search_read([("employee_id", "=", employee.id)])
        )
        dataset = []
        for rec in skills:
            vals = {
